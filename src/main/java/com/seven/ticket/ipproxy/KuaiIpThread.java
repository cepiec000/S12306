 ...
 * @Version V1.0
 **/
public class KuaiIpThread implements Runnable, IpLoadInterface {
    ...
    public List<HtmlElement> getHtml() {
        ...
        try {
            for (int i=1;i<3;i++){
                HtmlPage page = webClient.getPage(Constants.KDLNN.replace("{page}",i+""));
                List<HtmlElement> list1 = page.getByXPath("//*[@id=\"ip_list\"]/tbody/tr[position()>1]");
                all.addAll(list1);

                HtmlPage page2 = webClient.getPage(Constants.KDLNT.replace("{page}",i+""));
