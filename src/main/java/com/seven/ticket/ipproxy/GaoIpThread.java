 ...
 * @Version V1.0
 **/
public class GaoIpThread implements Runnable, IpLoadInterface {
    ...
    public List<HtmlElement> getHtml() {
        ...
        try {
            for (int i=1;i<3;i++){
                HtmlPage page = webClient.getPage(Constants.GEONN.replace("{page}",i+""));
                List<HtmlElement> list1 = page.getByXPath("/html/body/div/div[2]/div[1]/div[1]/table/tbody/tr[position()>1]");
                all.addAll(list1);

                HtmlPage page2 = webClient.getPage(Constants.GEONT.replace("{page}",i+""));
                List<HtmlElement> list2 = page2.getByXPath("/html/body/div/div[2]/div[1]/div[1]/table/tbody/tr[position()>1]");
                all.addAll(list2);
            }
